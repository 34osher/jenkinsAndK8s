...
API_KEY = 'c19ff9b37a4143f38de02bdeb26ed56f'
...
def filter_data(data_api):
    ...
    #######################################
    save_search_query(data_api['city_name'], weather_data)
    ###########################################
    return weather_data
...
def index():
    ...
    if request.method == 'POST':
        ...
        if weather_data is None:
            error_message = "Error getting weather data. Please try again."
    ##########################################################################################        
    bg_color = os.getenv('BG_COLOR', '#fd0b0b')  # Default color if env variable is not set
    return render_template('index.html', error_message=error_message, weather_data=weather_data, bg_color=bg_color)
#############################################################3
def save_search_query(city, data):
    directory = "search_history"
    if not os.path.exists(directory):
        os.makedirs(directory)
    file_name = f"{directory}/{city}_{datetime.now().strftime('%Y%m%d')}.json"
...
@app.route('/history')
def history():
    history_files = os.listdir('search_history')
    bg_color = os.getenv('BG_COLOR', '#fd0b0b')  # Default color if env variable is not set
    return render_template('history.html', history_files=history_files, bg_color=bg_color)
